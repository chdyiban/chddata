...
class Sign extends Api {
  ...
  const TIME_INTERVAL = 60;
	...
	public function init() {
		...
		if ($stuInfo != false) {
			$model       = new BaseModel;
			$stuBaseInfo = $model->getBaseInfoById($stuInfo->
								 yb_studentid);

			$this->initData['status']        = 'true';
			$personalInitData['yb_realname'] = $stuInfo->
								   yb_realname;
			$personalInitData['yb_id']       = $stuInfo->yb_userid;
			$personalInitData['stu_id']      = $stuInfo->
								  yb_studentid;

			$personalInitData['head_img']    = $stuInfo->
								   yb_userhead;
			$personalInitData['sign_status'] = $this->
				 getUserSignStatus($personalInitData['stu_id']);

			if ($stuBaseInfo['major']) {
				$personalInitData['major'] = $stuBaseInfo[
								       'major'];
			}
			if ($stuBaseInfo['college']) {
				$personalInitData['college'] = $stuBaseInfo[
								     'college'];
			}

			$this->initData['personal'] = $personalInitData;
			$this->initData['public']   = $this->getSign(
						   $personalInitData['stu_id']);
	...
	public function submit() {
		...
		$latitude  = $this->checkLocationValid(input('post.latitude',
									 '0'));
		$longitude = $this->checkLocationValid(input('post.longitude',
									  '0'));
		...
		if ($time - $this->getUserSignTimeStamp($stu_id) < self::TIME_INTERVAL ) {
			...
			$status = $this->getUserSignStatus($stu_id);
			if ($status == 2 || $status == 0) {
				$taskId = $this->getSignTaskId($stu_id);
				if ($this->recordSignData($stu_id, $latitude,
							 $longitude, $taskId)) {
					$data['status'] = 'success';
					$data['info']   = '签到成功';
				} else {
	...
	private function getSign($stu_id) {
		...
		if ($sign['task_status'] == 0) {
			...
			$signData['start_time']  = date("m-d H:i", $sign[
								'start_time']);
			$signData['end_time']    = date("m-d H:i", $sign[
								  'end_time']);
			$signData['notice']      = $this->getSignNotice($sign[
									'id']);
	...
	private function getSignTask($stu_id, $last = false) {
		$time           = time();
		$todayStartTime = mktime(0, 0, 0, date("m", $time), date("d",
						     $time), date("Y", $time));
		$todayEndTime   = mktime(23, 59, 59, date("m", $time), date("d"
						   , $time), date("Y", $time));
