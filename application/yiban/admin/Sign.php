...
use app\yiban\model\Sign as SignModel;
...
class Sign extends Admin
{
    ...
    public function signSpecial(){
        ...
        return ZBuilder::make('table')
                ...
                ->addTopSelect('task_id', '',$sign_task_list,$taskId)//添加顶部筛选
                ->addRightButton('edit',['href' => url('signSpecialList',['class_id' => '__class__','task_id'=> '__task_id__']),'title' => '详情','icon'=>'fa fa-fw fa-th-list'],'')
                ->setRowList($data) // 设置表格数据
                ->setPageTitle('特殊情况列表')
                ->fetch();
    }
    ...
    */
    public function getSignRateByClassId($class_id, $task_id){
    ...
    public function signSpecialList($class_id, $task_id){
